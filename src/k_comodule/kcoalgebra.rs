...
use ahash::HashMap;
use algebra::{abelian::Abelian, field::Field, matrices::flat_matrix::FlatMatrix, matrix::Matrix, ring::CRing, rings::finite_fields::F2};
...
pub fn A0_coalgebra() -> kCoalgebra<UniGrading, F2, FlatMatrix<F2>> {
    ...
    space.0.insert(
        UniGrading(0),
        vec![BasisElement { name: "1".to_owned(), excess: 0 }],
    );

    space.0.insert(
        UniGrading(1),
        vec![BasisElement { name: "xi1".to_owned(), excess: 0 }],

    );

    let mut coaction = HashMap::default();
    coaction.insert((UniGrading::zero(), 0), vec![((UniGrading::zero(), 0),(UniGrading::zero(), 0), F2::one())]);
