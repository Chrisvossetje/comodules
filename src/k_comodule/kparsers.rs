...
use crate::{
    ...
    types::{BasisElement, CoalgebraIndex, CoalgebraIndexType, ComoduleIndex, ComoduleIndexType},
};
...
impl<G: Grading, F: Field, M: Matrix<F>> kCoalgebra<G, F, M> {
    ...
    pub fn parse(
        input: &str,
        max_grading: G,
    ) -> Result<(kCoalgebra<G, F, M>, HashMap<String, CoalgebraIndex<G>>), String> {
    ...
    fn parse_direct(
        ...
        let mut basis_dict: HashMap<String, (BasisElement, G)> = HashMap::default();
        ...
        for (b, ls) in coaction_lut {
            ...
            for (scalar, l, r) in ls {
                ...
                let t_id = tensor.construct[&(r_id.0, r_id.1 as ComoduleIndexType)]
                    [&(l_id.0, l_id.1 as CoalgebraIndexType)];
                if (t_id.0) != *gr {
                    return Err(format!(
    ...
    fn parse_polynomial_hopf_algebra(
            ...
            match line {
                ...
                _ => match state {
                    ...
                    State::Coaction => {
                        ...
                        let tensors: HashMap<(Monomial, Monomial), F> = tensors
                            .split('+')
                            .map::<Result<((Monomial, Monomial), F), String>, _>(|t| {
                                let (s,t) = match t.split_once('.') {
                                    Some((s, t)) => {
        ...
        for (id, gen_coact) in (&mut coactions).iter_mut().enumerate() {
            ...
            let mut b = vec![0; n];
            b[id] = 1;
            gen_coact.insert((a.clone(), b.clone()), F::one());
            gen_coact.insert((b, a), F::one());
        }

        // Initialize basis information for the unit monomial (1)
        let mut one_coact = HashMap::default();
        one_coact.insert((one_monomial.clone(), one_monomial.clone()), F::one());
        ...
        let mut basis: HashMap<G, Vec<BasisElement>> = HashMap::default();
        ...
        for monomial in monomial_coaction.keys().sorted() {
            ...
            let element = BasisElement {
                name: label.clone(),
                excess: excess(monomial),
            };

            let index = basis.entry(grade).or_insert_with(Vec::new).len();
            monomial_to_grade_index.insert(monomial.clone(), (grade, index as CoalgebraIndexType));

            let index = basis.get(&grade).map_or(0, |el| el.len());
            basis_translate.insert(label, (grade, index as CoalgebraIndexType));
...
type HelperTensor<F> = HashMap<(Monomial, Monomial), F>;

fn excess(m: &Monomial) -> usize {
    m.iter().sum()
}
...
fn multiply_tensor_terms<F: Field>(
    a: (&(Monomial, Monomial), &F),
    b: (&(Monomial, Monomial), &F),
    relations: &Vec<Monomial>,
) -> Option<((Monomial, Monomial), F)> {
...
fn multiply_coaction_elements<F: Field>(
    ...
    result.retain(|_, b| *b != F::zero());
    result
}

fn get_list<G: Grading, F: Field, M: Matrix<F>>(
    map: &GradedLinearMap<G, F, M>,
    tensor: &TensorMap<G>,
    i: CoalgebraIndex<G>,
) -> Vec<(CoalgebraIndex<G>, CoalgebraIndex<G>, F)> {
    let m = &map.maps[&i.0];
    let mut v = vec![];
...
impl<G: Grading, F: Field, M: Abelian<F>> kComodule<G, kCoalgebra<G,F,M>> {
    ...
    fn parse_direct(
        ...
        // Create basis dictionary
        let mut basis_dict: HashMap<String, G> = HashMap::default();
        for (name, grade) in basis.iter() {
            if basis_dict.contains_key(name) {
                return Err(format!("Basis element '{}' appears twice", name));
            }
    ...
    fn parse_polynomial(
        ...
        // Construct the basis structure
        let mut basis: HashMap<G, Vec<_>> = HashMap::default();
        let mut monomial_to_grade_index: HashMap<Monomial, ComoduleIndex<G>> = HashMap::default();
                ...
                match coalgebra_translate.get(coalg_name) {
                    Some(coalg_index) => {
                        let (_, tensor_index) = tensor
                            .construct
                            .get(mod_grade_index)
                            .ok_or("Module CoalgebraIndex  not found in tensor construction.")?
                            .get(coalg_index)
                            .ok_or("Algebra CoalgebraIndex not found in tensor construction")?;
