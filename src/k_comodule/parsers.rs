...
use crate::{
    ...
    k_comodule::{
        ...
        kcoalgebra::kBasisElement,
    },
    types::{CoalgebraIndex, CoalgebraIndexType, ComoduleIndexType},
};
        ...
        for (b, ls) in coaction_lut {
            ...
            for (scalar, l, r) in ls {
                ...
                let t_id = tensor.construct[&(r_id.0, r_id.1 as ComoduleIndexType)]
                    [&(r_id.0, r_id.1 as ComoduleIndexType)];
                if (t_id.0) != *gr {
                    return Err(format!(
...
fn _multiply_comodule_coaction_elements<F: Field, G: Grading>(
